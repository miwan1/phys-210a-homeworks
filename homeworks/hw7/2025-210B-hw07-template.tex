...
\answer{ We first look at the two right eigenvectors in part (c).
    \be WP^\star = \begin{pmatrix} {1\over 2} & 0 \\ {1\over 2} & 1 \end{pmatrix} \begin{pmatrix} 0 \\ 1 \end{pmatrix} = \begin{pmatrix} 0 \\ 1 \end{pmatrix} = P^\star . \ee
    $P^\star$ is a steady state. 
    For detailed balance, we need to check that $W_{ij} P^\star_j = W_{ji} P^\star_i$ for all $i,j$.  The only nontrivial case is $i=1,j=2$:
    \be W_{12} P^\star_2 = 0 \cdot 1 = 0 , ~~~ W_{21} P^\star_1 = {1\over 2} \cdot 0 = 0 . \ee
    Yes, detailed balance is satisfied.
}
...
\answer{ The eigenvalues are $\lambda_1 = 1$ and $\lambda_2 = {1\over 2}$.  The corresponding right eigenvectors are
\be p^\star = \begin{pmatrix} 0 \\ 1 \end{pmatrix} , ~~~ \tilde p = \begin{pmatrix} 1 \\ -1 \end{pmatrix} . \ee
The steady state is $p^\star$.  }
...
\answer{
    \be p_0 = A \begin{pmatrix} 0 \\ 1 \end{pmatrix} + B \begin{pmatrix} 1 \\ -1 \end{pmatrix} = \begin{pmatrix} B \\ A - B \end{pmatrix} . \ee
For this to be a valid probability distribution, we need $B \geq 0$ and $A - B \geq 0$, or $A \geq B \geq 0$.  Also, we need $B + (A - B) = A = 1$.  So the conditions are $1 \geq B \geq 0$.
The time evolution is
\be p_n = W^n p_0 = A p^\star + B \lambda_2^n \tilde p = \begin{pmatrix} B (1/2)^n \\ 1 - B (1/2)^n \end{pmatrix} . \ee
}
